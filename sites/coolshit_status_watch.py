...
title across BOTH the /category/pokemon page and the general /products
page (since some items are occasionally miscategorized), not just ones
explicitly marked "coming soon". Alerts on any meaningful change - a
status flip, OR a title change on a URL we'd already seen. Uses the
shared retry helper for resilience.
"""
...
def _scrape_page(page, url, require_pokemon_keyword):
    ...
    for card in cards:
        ...
        entries[product_url] = {"title": title, "status": status, "price": price}
    return entries
...
def _try_fetch_once() -> list[dict]:
    ...
    with sync_playwright() as p:
        ...
        category_entries = _scrape_page(page, LISTING_URLS[0], require_pokemon_keyword=False)
        general_entries = _scrape_page(page, LISTING_URLS[1], require_pokemon_keyword=True)
        browser.close()

    combined = dict(category_entries)
    for url, entry in general_entries.items():
        combined.setdefault(url, entry)

    for product_url, entry in combined.items():
        title = entry["title"]
        status = entry["status"]
        price = entry["price"]
